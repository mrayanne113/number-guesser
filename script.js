...
var randomNumber = 0;
var userMax;
var userMin = document.querySelector('.new-min').value;
...
function feedbackOfGuess(guessParse, userMin, userMax) {
  console.log(userMax, userMin)
...
function resetTheGame() {
  ...
  levelUpBttn.classList.add('display-none');
...
function userRandomNumber() { 
  ...
  document.querySelector('.new-range').innerText = 'Range ' + min + ' - ' + max;
...
function openByTen() {
  ...
  document.querySelector('.new-range').innerText = 'Range ' + parseMin + ' - ' + parseMax;
  randomNumber = Math.floor(Math.random() * (parseMax - parseMin) + parseMin); 
